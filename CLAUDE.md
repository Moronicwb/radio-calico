...
- **Frontend:** Plain HTML/CSS/JS in `public/`; minified to `dist/` by esbuild for production
...
- `scripts/build.js` — esbuild pipeline: minifies `public/app.js` + `public/styles.css` into `dist/`, copies other assets
...
- `nginx/nginx.conf` — nginx: serves `dist/` with gzip + long-term cache headers, proxies `/api/` to Express
- `Makefile` — make targets: `prod`, `dev`, `build`, `test`, `test-coverage`, `security`, `db-up`, `db-down` (postgres only), `db-migrate`, `db-studio`, `down` (all containers)
- `Dockerfile` — multi-stage: `dev` (hot reload), `build` (runs esbuild), `prod` (Express API), `nginx-prod` (serves minified dist/)
...
- Edit source files in `public/` — `dist/` is generated and gitignored. The build runs automatically inside Docker for `make prod`.
